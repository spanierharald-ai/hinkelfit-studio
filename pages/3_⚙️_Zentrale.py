...
def get_max_sessions(tarif):
    ...
    # Extrahiert die Stunden aus der dynamischen Firmenfitness-Eingabe (z.B. "2 Std./Woche")
    match = re.search(r'(\d+)\s*Std\./Woche', tarif_str)
    if match:
        return int(match.group(1))
        
    if "1x" in tarif_str: return 1
    if "2x" in tarif_str: return 2
    if "Firmenfitness" in tarif_str: return 99 # Fallback, falls kein Wert gefunden wird
